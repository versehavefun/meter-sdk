...
func NewMainnet() *Genesis {
	...
	builder := new(Builder).
		...
		State(func(state *state.State) error {
			...
			state.SetCode(builtin.Meter.Address, builtin.Meter.RuntimeBytecodes())               // new addr 0x000000000000000000004d657465724552433230
			state.SetCode(builtin.MeterGov.Address, builtin.MeterGov.RuntimeBytecodes())         // new addr 0x000000000000004d65746572476f764552433230
			state.SetCode(builtin.MeterTracker.Address, builtin.MeterTracker.RuntimeBytecodes()) // new addr 0x0000000000004E65774D657465724E6174697665
			...
			// pre-allocate for Executor address
			state.SetBalance(builtin.Executor.Address, big.NewInt(1).Mul(big.NewInt(100), big.NewInt(1e18)))
			state.SetEnergy(builtin.Executor.Address, big.NewInt(1).Mul(big.NewInt(100), big.NewInt(1e18)))
